...
def nn(X, Y):
	...
	with open(log_path, 'w') as f:
		f.write('train_accuracy,test_accuracy,' +
		        'pca_train_accuracy,pca_test_accuracy,' +
		        'ica_train_accuracy,ica_test_accuracy,' +
		        'rca_train_accuracy,rca_test_accuracy,' +
		        'rf_train_accuracy,rf_test_accuracy,'
		        'km_train_accuracy,km_test_accuracy,' +
		        'pca_km_train_accuracy,pca_km_test_accuracy,' +
		        'ica_km_train_accuracy,ica_km_test_accuracy,' +
		        'rca_km_train_accuracy,rca_km_test_accuracy,' +
		        'rf_km_train_accuracy,rf_km_test_accuracy,'+
		        'gmm_train_accuracy,gmm_test_accuracy,' +
		        'pca_gmm_train_accuracy,pca_gmm_test_accuracy,' +
		        'ica_gmm_train_accuracy,ica_gmm_test_accuracy,' +
		        'rca_gmm_train_accuracy,rca_gmm_test_accuracy,' +
		        'rf_gmm_train_accuracy,rf_gmm_test_accuracy,'+
		        '\n')
	...
	for train_index, test_index in kf.split(X):
		...
		for i in range(0, 5):
			...
			km_train_acc[fold, i], km_test_acc[fold, i] = kmeans_train_predict(k_for_kmeans[i], hidden_layer_sizes,
			                                                                   TX_train, Y_train, TX_test, Y_test)
			gmm_train_acc[fold, i], gmm_test_acc[fold, i] = gmm_train_predict(k_for_kmeans[i], hidden_layer_sizes,
			                                                                   TX_train, Y_train, TX_test, Y_test)
	...
	with open(log_path, 'a') as f:
		output = ''
		for i in range(0, 5):
			output = output + ',{},{}'.format(trans_train_acc[i], trans_test_acc[i])
		for i in range(0, 5):
			output = output + ',{},{}'.format(km_train_acc[i], km_test_acc[i])
		for i in range(0, 5):
			output = output + ',{},{}'.format(gmm_train_acc[i], gmm_test_acc[i])
